...
def etcd_initial_cluster
  (1..$num_instances).map { |i| "%s-%02d=http://%s:2380" % [ $instance_name_prefix, i, etcd_ip(i) ] }.join(",")
end

def etcd_initial_cluster_token
  ENV['CLUSTER_NAME'] || 'lab'
end

def etcd_initial_cluster_state
  ENV['ACTION'] == 'add' ? 'existing' : 'new'
end

def cloud_config_file
  "user-data"
end
...
Vagrant.configure("2") do |config|
  ...
  (1..$num_instances).each do |i|
    config.vm.define vm_name = "%s-%02d" % [$instance_name_prefix, i] do |config|
      ...
      data['coreos']['etcd2']['initial-cluster'] = etcd_initial_cluster
      data['coreos']['etcd2']['initial-cluster-state'] = etcd_initial_cluster_state
      data['coreos']['etcd2']['initial-cluster-token'] = etcd_initial_cluster_token
      ...
      if File.exist?(etcd_config_file.path)
